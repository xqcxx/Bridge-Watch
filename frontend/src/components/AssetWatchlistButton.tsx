import { useWatchlist } from "../hooks/useWatchlist";

interface AssetWatchlistButtonProps {
  symbol: string;
  className?: string;
  size?: "sm" | "md" | "lg";
}

export function AssetWatchlistButton({ symbol, className = "", size = "md" }: AssetWatchlistButtonProps) {
  const { activeWatchlist, addAsset, removeAsset } = useWatchlist();

  const isTrailing = activeWatchlist?.assets.includes(symbol);

  const toggleWatchlist = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    
    if (!activeWatchlist) return;

    if (isTrailing) {
      removeAsset.mutate({ watchlistId: activeWatchlist.id, symbol });
    } else {
      addAsset.mutate({ watchlistId: activeWatchlist.id, symbol });
    }
  };

  const iconSizes = {
    sm: "w-4 h-4",
    md: "w-5 h-5",
    lg: "w-6 h-6",
  };

  return (
    <button
      onClick={toggleWatchlist}
      className={`focus:outline-none transition-colors 
      ${isTrailing ? "text-yellow-400 hover:text-yellow-300" : "text-gray-400 hover:text-white"} 
      ${className}`}
      title={isTrailing ? "Remove from Watchlist" : "Add to Watchlist"}
      aria-label={isTrailing ? `Remove ${symbol} from Watchlist` : `Add ${symbol} to Watchlist`}
    >
      <svg 
        className={iconSizes[size]} 
        fill={isTrailing ? "currentColor" : "none"} 
        viewBox="0 0 24 24" 
        stroke="currentColor" 
        strokeWidth="2"
      >
        <path strokeLinecap="round" strokeLinejoin="round" 
          d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" 
        />
      </svg>
    </button>
  );
}
